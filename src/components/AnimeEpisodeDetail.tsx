...
const Content: FC<Props> = ({ share_title, share_url, episode: props }) => {
  ...
  const link_facebook: string =
    'https://www.facebook.com/dialog/feed?app_id=1846956072250071&link=' +
    encodeURIComponent(share_url)
  const link_line: string =
    'http://line.me/R/msg/text/?' + encodeURIComponent(share_url)
  ...
  if (props.detail.characters.length > 0) {
    ...
    character = (
      <div className='box'>
        <h2>主な登場人物</h2>
        <div className='character'>{character_tags}</div>
      </div>
    )
  }
  ...
  if (props.id > 1) {
    ...
    arrow_prev = (
      <Link href={'/anime/1/' + --prev_id} className='arrow-prev'>
        前のエピソード
      </Link>
    )
  }
  ...
  if (props.id < final_eqisode_no) {
    ...
    arrow_next = (
      <Link href={'/anime/1/' + ++next_id} className='arrow-next'>
        次のエピソード
      </Link>
    )
  }
      ...
      <div className='wrapper'>
        <article className='episode-box'>
          <div className='header'>
            ...
            <div className='onair'>
              {formatDateJP(new Date(props.onair_date))}放送（第{props.onair_no}
              回放送）
            </div>
          </div>
          {youtube}
