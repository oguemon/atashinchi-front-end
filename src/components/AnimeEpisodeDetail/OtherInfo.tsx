...
const Content: FC<Props> = ({ episode }) => {
    ...
    const amzn_btn = (amzn_no == 0)? <></> : <a className="ondemand-link" href={ amzn_url } rel="noreferrer" target="_blank">視聴する</a>;

    // Abema TVの話数とリンクを求める
    let abema_no =  episode.detail!.amzn_no;
    let abema_url = "https://abema.tv/video/episode/35-34_s1_p" + String(abema_no);

    if (episode.detail!.amzn_no <= 198) { // 198話以前は3エピソードで1話となり、URLも異なる
    ...
    const abema_btn = (abema_no == 0)? <></> : <a className="ondemand-link" href={ abema_url } rel="noreferrer" target="_blank">視聴する</a>;
        ...
        <div className="box">
            ...
            <table className="other-info">
                <tbody>
                    ...
                    <tr>
                        <th>ビデオ</th>
                        <td colSpan={2}>第{ episode.video.collection }集・Vol.{ episode.video.volume }</td>
                    </tr>
                </tbody>
            </table>
        </div>
